...
static u32 sunh5_ui_scaler_base(struct sunh5_mixer *mixer, int channel)
{
...
static int sunh5_ui_scaler_coef_index(unsigned int step)
{
	unsigned int scale, int_part, float_part;

	scale = step >> (SUNH5_UI_SCALER_SCALE_FRAC - 3);
	int_part = scale >> 3;
...
void sunh5_ui_scaler_setup(struct sunh5_mixer *mixer, int layer, 
							...
							u32 hscale, u32 vscale, u32 hphase, u32 vphase)
{
	...
	base = sunh5_ui_scaler_base(mixer, layer);

	hphase <<= SUNH5_UI_SCALER_PHASE_FRAC - 16;
	vphase <<= SUNH5_UI_SCALER_PHASE_FRAC - 16;
	hscale <<= SUNH5_UI_SCALER_SCALE_FRAC - 16;
	vscale <<= SUNH5_UI_SCALER_SCALE_FRAC - 16;

	insize = SUNH5_UI_SCALER_SIZE(src_w, src_h);
	outsize = SUNH5_UI_SCALER_SIZE(dst_w, dst_h);

	regmap_write(mixer->engine.regs, SUNH5_SCALER_GSU_OUTSIZE(base), outsize);
	regmap_write(mixer->engine.regs, SUNH5_SCALER_GSU_INSIZE(base), insize);
	regmap_write(mixer->engine.regs, SUNH5_SCALER_GSU_HSTEP(base), hscale);
	regmap_write(mixer->engine.regs, SUNH5_SCALER_GSU_VSTEP(base), vscale);
	regmap_write(mixer->engine.regs, SUNH5_SCALER_GSU_HPHASE(base), hphase);
	regmap_write(mixer->engine.regs, SUNH5_SCALER_GSU_VPHASE(base), vphase);
	offset = sunh5_ui_scaler_coef_index(hscale) * SUNH5_UI_SCALER_COEFF_COUNT;
	for(i = 0; i < SUNH5_UI_SCALER_COEFF_COUNT; i++)
		regmap_write(mixer->engine.regs,
						SUNH5_SCALER_GSU_HCOEFF(base, i),
						lan2coefftab16[offset + i]);
}

void sunh5_ui_scaler_enable(struct sunh5_mixer *mixer, int layer, bool enable)
{
	u32 val, base;
	...
	if(enable)
		val = SUNH5_SCALER_GSU_CTRL_EN | SUNH5_SCALER_GSU_CTRL_COEFF_RDY;
	else
		val = 0;

	regmap_write(mixer->engine.regs, SUNH5_SCALER_GSU_CTRL(base), val);
}
